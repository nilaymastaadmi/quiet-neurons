...
| `experiments/results/contrast.md`, `experiments/results/latency.md`, `experiments/results/word_scatter.README.md`, `experiments/results/pool_ladder.README.md`, `experiments/results/transformer_control.README.md`, `experiments/results/one_period_vs_two.md`, `experiments/results/seed_variance.README.md`, `experiments/results/onepager_test.md`, `experiments/results/full_schedule.README.md` | Method notes for the three page-driven measurements (WCAG contrast; interaction latency; per-word injection scatter), each with the script that reproduces it | MIT, `../LICENSE` |
...
| `experiments/transformer_control.py` | The non-BDH control: a dense Transformer trained on the identical task with the same seed, optimiser, 4,000-step schedule and `--stop-at 2309`, measured on the fraction of post-ReLU MLP units above zero. Written for this submission | MIT, `../LICENSE` |
| `experiments/stepmatched/*.pid` | Provenance of the long training runs: the process ids a run was launched under, the reason it was restarted, and the relaunch command. `train_n8192_full4000.killed-at-step600.log` is the preserved partial log of the run that died at step 600 of 4,000 when its wrapper exited and reaped it  **Every log in this package has had one repeated two-line torch warning removed** ("Failed to initialize NumPy", which the measurement venv emits at import). It said nothing about any run, was identical in all seventeen files, and printed an absolute local path into a shipped artifact. 21 blocks went; every step, loss, timing, TRAINED and PRECONDITION line is untouched. `train_n8192_full4000.err` contained nothing but that warning and is no longer shipped. | MIT, `../LICENSE` |
