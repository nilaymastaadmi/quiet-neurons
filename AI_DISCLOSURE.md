...
| `experiments/results/contrast.md`, `experiments/results/word_scatter.README.md` | Method notes for the two measurements taken by driving the running page rather than by `measure.py`: WCAG contrast, and the per-word surprise-injection scatter. Both carry the console script that reproduces them | MIT, see `LICENSE` |
