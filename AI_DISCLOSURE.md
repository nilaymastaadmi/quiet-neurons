...
| `experiments/stepmatched/*.pid`, `experiments/stepmatched/*.err` | Provenance of the long training runs: the process ids a run was launched under, the reason it was restarted, and the relaunch command. `train_n8192_full4000.killed-at-step600.log` is the preserved partial log of the run that died at step 600 of 4,000 when its wrapper exited and reaped it | MIT, see `LICENSE` |
